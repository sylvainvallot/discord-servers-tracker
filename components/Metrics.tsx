export default function Metrics({ data }){
    ...
    datasets.map(entry => {
        ranking.push({label: entry.label, metric: getWeekMemberMetric(entry, weekly)})
    })
    ...
    function getWeekSpan(entry, timeSpan){
        ...
        return `${new Date(prevEntry.x).toLocaleDateString(undefined, options)} au ${new Date(lastEntry.x).toLocaleDateString(undefined, options)}`
    }
    ...
    function MemberMetric({data, rank}){
        ...
        function Chevron({number, className}){
            if(number == 0) return (
        ...
        return (
            <div className={'p-3 flex flex-row items-center justify-between my-2 border-b'}>
                <div className='flex flex-row items-center gap-4'>
                    ...
                    <div className="flex flex-row items-center">
                        ...
                        <div className={"font-bold text-2xl m-1 " + numberColor(data.metric)}>{data.metric}</div>
                    </div>
            </div>
        )
    };
    ...
    return (
        <div className='container p-12'>
            ...
            <h4 className="italic">Sur la période du {getWeekSpan(datasets[0], weekly)} - Exprimé en nouveau membres</h4>
